...
btnRegister.addEventListener("click", function (e) {
    ...
    if (!passwdagain.value || passwdagain.value != passwd.value) {
        ...
        passwdagain.style.border =
            "1px solid var(--register-screen-color-input)";
    }
...
async function enviar_formulario() {
    able = true;
    await fetch(
        "https://api.talkandeat.es/api/users?username=" + username.value
    )
        ...
        .then((data) => {
            if (!data["msg"]) {
                ...
                username.style.border =
                    "1px solid var(--register-screen-error)";
                able = false;
            }
        });

    await fetch("https://api.talkandeat.es/api/users?email=" + email.value)
        .then((response) => response.json())
        .then((data) => {
            if (!data["msg"]) {
